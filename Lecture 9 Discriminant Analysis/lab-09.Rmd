...
prob.good <- exp(pred$scores[,2])/(exp(pred$scores[,1])+exp(pred$scores[,2]))
prob.good 
...
plot (c(0,gain$cume.pct.of.total*sum(df[test.index,3]=="Yes"))~ c(0,gain$cume.obs),xlab="cumulative number of administrators", ylab="cumulative number of good administrators",type="l")
